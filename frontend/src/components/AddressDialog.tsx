...
export function AddressDialog({icon, userId, setAddresses}:{icon:string, userId:number, setAddresses:React.Dispatch<React.SetStateAction<Address[]>>}) {
...
function changeAddressForm(event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) {
  setAddressForm({
    ...addressForm,
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4">
            <div className="grid gap-3">
              ...
              <Input id="address" name="address" placeholder="Av.Brasil " onChange={changeAddressForm} value={addressForm.address} />
            </div>
       ...
       <RadioGroup value={addressForm.addressType} onValueChange={(value)=>{
        setAddressForm({
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={async () => { await saveAddress(addressForm); setAddresses(await getAddressesByUserId(userId)) }}>Save address</Button>
          </DialogFooter>
        </DialogContent>
      </form>
    </Dialog>
